import { useState } from "react";
import { supabase } from "../lib/supabase";
import { Heart } from "lucide-react";

type Mode = "signin" | "signup";

export default function Auth() {
  const [mode, setMode] = useState<Mode>("signin");

  const handleGoogleAuth = () => {
    supabase.auth.signInWithOAuth({ provider: "google" });
  };

  return (
    <div className="min-h-screen flex bg-[#F5F7FB]">
      {/* Left panel — brand showcase */}
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-[#2F6FED] to-[#1E4FBE] relative overflow-hidden items-center justify-center">
        <div
          className="absolute inset-0 opacity-[0.04]"
          style={{
            backgroundImage:
              "radial-gradient(circle at 25% 50%, white 1px, transparent 1px)",
            backgroundSize: "32px 32px",
          }}
        />
        <div className="relative z-10 text-center px-12 max-w-md">
          <div className="w-16 h-16 rounded-xl bg-white flex items-center justify-center mx-auto mb-6 shadow-lg">
            <Heart className="w-8 h-8 text-[#2F6FED] fill-[#2F6FED]" />
          </div>
          <h1 className="text-3xl font-bold text-white leading-tight">
            Your Health,
            <br />
            Our Priority
          </h1>
          <p className="text-white/70 text-sm mt-3 leading-relaxed">
            Smart tools for a healthier you. Manage your health records, get
            AI-powered insights, and find nearby care.
          </p>
        </div>
      </div>

      {/* Right panel — auth card */}
      <div className="flex-1 flex items-center justify-center px-6">
        <div className="w-full max-w-sm">
          {/* Logo */}
          <div className="flex items-center gap-2.5 mb-8 justify-center lg:justify-start">
            <div className="w-10 h-10 rounded-lg bg-[#2F6FED] flex items-center justify-center shrink-0">
              <Heart className="w-5 h-5 text-white fill-white" />
            </div>
            <span className="text-lg font-bold tracking-tight">
              <span className="text-[#111827]">Ben</span>
              <span className="text-[#2F6FED]">Healthy</span>
            </span>
          </div>

          <div className="bg-white rounded-2xl shadow-[0_1px_3px_rgba(16,24,40,0.06)] border border-[#E5E7EB] p-8">
            {/* Toggle */}
            <div className="flex items-center gap-1 bg-gray-100 rounded-full p-1 mb-6">
              <button
                onClick={() => setMode("signin")}
                className={`flex-1 px-4 py-1.5 text-sm font-medium rounded-full transition-colors ${
                  mode === "signin"
                    ? "bg-[#2F6FED] text-white shadow-sm"
                    : "text-gray-500 hover:text-gray-700"
                }`}
              >
                Sign In
              </button>
              <button
                onClick={() => setMode("signup")}
                className={`flex-1 px-4 py-1.5 text-sm font-medium rounded-full transition-colors ${
                  mode === "signup"
                    ? "bg-[#2F6FED] text-white shadow-sm"
                    : "text-gray-500 hover:text-gray-700"
                }`}
              >
                Sign Up
              </button>
            </div>

            {mode === "signin" ? (
              <>
                <h2 className="text-2xl font-bold text-[#111827]">
                  Welcome back
                </h2>
                <p className="text-sm text-[#6B7280] mt-1.5">
                  Sign in to manage your health records and get AI-powered
                  insights.
                </p>
              </>
            ) : (
              <>
                <h2 className="text-2xl font-bold text-[#111827]">
                  Create your account
                </h2>
                <p className="text-sm text-[#6B7280] mt-1.5">
                  Join HealthCompanion to track your health records and get
                  AI-powered insights.
                </p>
              </>
            )}

            <div className="mt-8 space-y-3">
              <button
                type="button"
                onClick={handleGoogleAuth}
                className="w-full flex items-center justify-center gap-2.5 px-4 py-2.5 bg-[#2F6FED] text-white text-sm font-semibold rounded-lg hover:bg-[#1E4FBE] transition-colors shadow-sm"
              >
                <svg
                  className="w-4 h-4 shrink-0"
                  viewBox="0 0 24 24"
                  fill="none"
                >
                  <path
                    d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
                    fill="#fff"
                  />
                  <path
                    d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                    fill="#fff"
                  />
                  <path
                    d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                    fill="#fff"
                  />
                  <path
                    d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                    fill="#fff"
                  />
                </svg>
                {mode === "signin"
                  ? "Sign in with Google"
                  : "Sign up with Google"}
              </button>
            </div>

            {mode === "signup" && (
              <p className="text-xs text-[#6B7280] mt-4 text-center">
                Already have an account?{" "}
                <button
                  onClick={() => setMode("signin")}
                  className="text-[#2F6FED] font-medium hover:underline"
                >
                  Sign in
                </button>
              </p>
            )}
            {mode === "signin" && (
              <p className="text-xs text-[#6B7280] mt-4 text-center">
                Don't have an account?{" "}
                <button
                  onClick={() => setMode("signup")}
                  className="text-[#2F6FED] font-medium hover:underline"
                >
                  Sign up
                </button>
              </p>
            )}

            <p className="text-xs text-[#6B7280] mt-4 text-center leading-relaxed">
              By continuing, you agree to our Terms of Service and Privacy
              Policy.
            </p>
          </div>
        </div>
      </div>
    </div>
  );
}
