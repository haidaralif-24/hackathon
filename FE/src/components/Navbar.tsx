...
const pageMeta: Record<string, { titleKey: string; subKey: string }> = {
  ...
  "/health-record": { titleKey: "nav_health_record", subKey: "nav_health_record_sub" },
...
export default function Navbar({ email, onSignOut, userName, userAvatar, onToggleSidebar }: NavbarProps) {
  ...
  return (
    <nav className="sticky top-0 z-30 w-full bg-white/80 backdrop-blur-sm border-b border-[#E5E7EB] h-16">
      <div className="flex items-center justify-between h-full px-4 md:ps-20 lg:ps-70">
        <button onClick={onToggleSidebar} className="md:hidden text-gray-500 hover:text-gray-700 mr-3 cursor-pointer" aria-label="Toggle sidebar">
        ...
        <div className="flex-1 lg:flex-none inline md:hidden">
          <h1 className="text-xl font-bold text-[#111827]">
            {t(meta.titleKey)}
            {location.pathname === "/" && (
              <><span className="inline md:hidden">, {firstName}!</span><span className="hidden md:inline">, {userName}!</span></>
            )}
          </h1>
          <p className="text-xs text-[#6B7280] hidden md:inline">{t(meta.subKey)}</p>
        </div>
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <MessageCircle className="w-5 h-5 text-gray-500 hover:text-[#2F6FED] cursor-pointer transition-colors" onClick={() => navigate("/chat")} />
            <span className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-[#2F6FED] rounded-full border-2 border-white" />
          </div>
          <div className="relative" ref={dropdownRef}>
            <button onClick={() => setDropdownOpen(!dropdownOpen)} className="flex items-center gap-2.5 cursor-pointer">
              ...
              <span className="hidden md:inline text-sm font-medium text-gray-700">{userName || email}</span>
              <ChevronDown className={`hidden lg:block w-3.5 h-3.5 text-gray-400 transition-transform ${dropdownOpen ? "rotate-180" : ""}`} />
            </button>
            {dropdownOpen && (
