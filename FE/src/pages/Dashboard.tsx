...
function getInitialTone(): string {
  ...
  if (raw === "Straightforward" || raw === "Langsung")
    return "tone_straightforward";
...
export default function Dashboard({ userName }: { userName?: string }) {
  ...
  const [customName, setCustomName] = useState(
    () => localStorage.getItem("custom_persona_name") || "",
  );
  const [customDesc, setCustomDesc] = useState(
    () => localStorage.getItem("custom_persona_desc") || "",
  );
  ...
  const personaLabel =
    tone === "tone_custom" && (customName || customDesc)
      ? customName || "Custom"
      : t(tone);
  const personaDesc =
    tone === "tone_custom"
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-4rem)] px-6">
      ...
      <p className="text-gray-500 text-lg mt-4">{t("home_welcome_desc")}</p>
